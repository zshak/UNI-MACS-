...
public class breakoutExtension extends GraphicsProgram {
	...
	private void intro() {
	//intro informs player that the game will start after a mouse click
		GLabel intro = text("Click To Start!");
		add(intro);
		waitForClick();
		remove(intro);
	//after a click intro is removed
	}
	...
	private Color color(int j) {
		/*bricks take color according to their row number
		 * 1 and 2 - red, 3 and 4 - orange, 5 and 6 - yellow, 7 and 8 - green, 9 and 10 - cyan
		 */
		if (j % 10 == 1 || j % 10 == 2) {
			return RED_CLR;
	...
	private void drawPaddle() {
		...
		Paddle.setColor(GREEN_CLR);
		Paddle.setFillColor(GREEN_CLR);
		add(Paddle);
	}

	public void mouseMoved(MouseEvent e) {
		/* only x coordinate of the mouse is considered when moving paddle
		 * if mouse moves so that paddle goes out of the window, it is automatically set in a correct location
		 */
		int x = e.getX();
	...
	private void drawBall() {
		...
		Ball.setFilled(true);
		Ball.setColor(RED_CLR);
		add(Ball);
	}
	
	private void drawPointCounter() {
		point= new GLabel("POINTS: " + points, 10,20);
		add(point);
	}

	private void moveBall() {
		/*ball is given a starting velocity randomly 
		 * it has to stop moving if the player has no turns left or if they have already won the game
		 */
	...
	private void ballMovement() {
		/*ball changes vx when it hits left or right wall
		 * it changes vy when it hits upper wall
		 * if it hits lower wall, the turns are decreased 
		 * the movement ends when there are no more turns or bricks left
		 */
		Ball.move(vx, vy);
		pause(10);
		breakBricks();
		...
		if (hitDownWall()) {
			...
			drawPaddle();
			drawBall();
		//when there are no more turns left, ready alert is no longer needed
			if (turns != 0) {
				getReady();
			}
			moveBall();
	...
	private void getReady() {
		//adds get ready alert
		GLabel ready = text("get ready: 3");
		add(ready);
		pause(800);
		ready.setLabel("get ready: 2");
		pause(800);
		ready.setLabel("get ready: 1");
		pause(800);
		remove(ready);
	}

	private GLabel text(String a) {
		//returns GLabel that is set up at certain coordinates
		GLabel text = new GLabel(a);
		text.setLocation(getWidth() / 2 - text.getWidth() / 2, getHeight() / 2 - text.getHeight());
		text.setColor(RED_CLR);
	...
	private GObject getCollidingObject() {
		//returns object that is at any one of the four rectangle points that surround circle
	...
	private void breakBricks() {
		/*if circle hits any object except paddle, that object is removed
		 * corners are considered points where ball's centre x coordinate is smaller or larger than paddle's x coordinates
		 * if the ball hits left corner and it's coming from left, ball's both coordinates are changed
		 * if the ball hits right corner and it's coming from right, same will happen
		 * if it hits paddle normally, only y coordinate changes
		 */
		...
		if (vy > 0 & pointCentreY < Paddle.getY() & collider == Paddle & (corner1 || corner2)) { 
			// vy>0 is needed so that ball doesn't get stuck in a paddle
			vy = -vy;
			vx = -vx;
		} else if(vy > 0 & pointCentreY>Paddle.getY()&collider == Paddle & (corner1 || corner2)){
			vx = -vx;
			...
			brickCount--;
			bounceClip.play();
			if(collider.getColor() == CYAN_CLR) {
				points+=10;
				point.setLabel("POINTS: " + points);
			}else if(collider.getColor() == GREEN_CLR) {
				points+=20;
				point.setLabel("POINTS: " + points);
			}else if(collider.getColor()==YELLOW_CLR) {
				points+=30;
				point.setLabel("POINTS: " + points);
			}else if(collider.getColor() == ORANGE_CLR) {
				points+=40;
				point.setLabel("POINTS: " + points);
			}else {
				points+=50;
