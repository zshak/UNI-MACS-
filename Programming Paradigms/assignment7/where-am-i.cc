...
(define (partition pivot num-list comp_fn)
  (if (null? num-list) '(() ())
      (let ((split-of-rest (partition pivot (cdr num-list) comp_fn)))
	(if (comp_fn (car num-list) pivot)
...
(define (quicksort num-list comp_fn)
  (if (<= (length num-list) 1) num-list
      (let ((split (partition (car num-list) (cdr num-list) comp_fn)))
	(append (quicksort (car split) comp_fn) 
		(list (car num-list)) 
		(quicksort (cadr split) comp_fn)))))
...
(define (intersection-points circles)
  (if(= (length circles) 1) '()
    (append (apply append 
        ...
        (map (lambda (x) (intersect (car circles) x)) (cdr circles ))) (intersection-points (cdr circles)))))
  

(define (distance-product point list_of_points)
  (apply * (map (lambda (x) (dist x point)) (remove point list_of_points)))

)

(define (rate-points list_of_points)
  (map (lambda (x) (list (distance-product x list_of_points) x)) list_of_points)
)

(define (sort-points list_of_points)
  (quicksort list_of_points (lambda (a b) (< (car a) (car b))))
)

(define (clumped-points list_of_points)
  (prefix-of-list (map (lambda (x) (cadr x)) (sort-points (rate-points list_of_points))) (floor( / (length list_of_points) 2)))
)

(define (average-point list_of_points)
  (let (
    (average
      (list (/ (apply + (map car list_of_points)) (length list_of_points)) (/ (apply + (map cadr list_of_points)) (length list_of_points))) 
    )
  )
  (list (distance-product average list_of_points) average)
  )
)

(define (best-estimate list_of_circles)
  (average-point
    (clumped-points
      (intersection-points list_of_circles) 
    )
  )
)

(define (where-am-i list_of_distances list_of_star_locations)
  (sort-points
    (map best-estimate (all-guesses list_of_distances list_of_star_locations))
  )
)
;;; --------------------------------------------------------------------------------------
;;; 				!!!!!!! TESTING PART !!!!!!!
