...
bool containsNonTerminal(vector<string> &generatedWords){
...
void generateSentenceRec(vector<string> &generatedWords, map<string,Definition>& grammar){
  if(!containsNonTerminal(generatedWords)) return;
  vector<string> temp;
  for(string word: generatedWords){
    if(word[0] == '<' && word[word.length() - 1] == '>'){ //non-terminal
      Definition wordDef = grammar[word];
      Production wordProd = wordDef.getRandomProduction();
      for(Production::iterator it = wordProd.begin(); it != wordProd.end(); ++it) temp.push_back(*it);
    }else{ //terminal
      temp.push_back(word);
    }
  }
  ...
  generatedWords = temp;
  generateSentenceRec(generatedWords, grammar);
}

void printText(vector<string>& generatedWords){
  for (int i = 0; i < generatedWords.size(); i++) {
    cout<< generatedWords[i]<< " ";
  }
  cout<<endl;
}
...
void generateSentence(map<string,Definition>& grammar)
{
  ...
  generatedWords.push_back("<start>");

  generateSentenceRec(generatedWords, grammar);

  printText(generatedWords);
}
...
int main(int argc, char *argv[])
{
  ...
  cout<<grammar["<start>"].getNonterminal()<<endl;
  for (int i = 0; i < 3; i++)
  {
