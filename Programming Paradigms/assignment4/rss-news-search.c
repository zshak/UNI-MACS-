...
static void BuildIndices(const char *feedsFileName, hashset *stopWords, hashset *usedUrl, hashset *answers);
static void ProcessFeed(const char *remoteDocumentName, hashset *stopWords, hashset *usedUrl, hashset *answers);
static void PullAllNewsItems(FILE *dataStream, hashset* stopWords, hashset* usedUrl, hashset* answers);
static bool GetNextItemTag(streamtokenizer *st);
static void ProcessSingleNewsItem(streamtokenizer *st, hashset* stopWords, hashset* usedUrl, hashset* answers);
...
static void ParseArticle(const char *articleTitle,
                         ...
                         const char *articleURL, hashset* stopWords, hashset* usedUrl, hashset* answers);
static void ScanArticle(streamtokenizer *st, const char *articleTitle,
                        const char *unused, const char *articleURL, hashset* stopWords, hashset* usedUrl, hashset* answers);
static void QueryIndices(hashset* stopwords, hashset* answers);
static void ProcessResponse(const char *word, hashset* stopwords, hashset* answers);
...
static const char *const kStopFile = "data/stop-words.txt";
...
static int ArticlesWordsCompareFn(const void *elem1, const void *elem2) {
  int words1 = ((Article*)elem1)->words;
  int words2 = ((Article*)elem2)->words;
  if(words2>words1) return 1;
  if(words2 == words1) return 0;
  return -1;
}

static void SortMapFn(void *elemAddr, void *auxData) {
  VectorSort(&((wordInfo*) elemAddr)->articles, ArticlesWordsCompareFn);
}

int StringHash(const void *el, int numBuckets)
{
  int i;
...
void freeWord(void *elem)
{
  wordInfo *word = (wordInfo *)elem;
  free(word->word);
  VectorDispose(&word->articles);
}

static int wordHash(const void *word, int numBuckets)  { 
  return StringHash(&(((wordInfo *)word)->word), numBuckets);                                
}

static int wordCmp(const void *word1, const void *word2) {
  return strcasecmp(((wordInfo*) word1)->word, ((wordInfo*) word2)->word);
}

static void initializeStopWord(hashset *set)
{
  HashSetNew(set, sizeof(char *), 10007, StringHash, stringComp, freeString);
  FILE *file = fopen(kStopFile, "r");
  assert(file != NULL);

  streamtokenizer tk;
  char word[30];
  STNew(&tk, file, kNewLineDelimiters, true);

  while (STNextToken(&tk, word, sizeof(word)))
  {
    char *temp = strdup(word);
    HashSetEnter(set, &temp);
  }
...
{
  hashset set;
  initializeStopWord(&set);

  hashset usedUrl;
  HashSetNew(&usedUrl, sizeof(char *), 10007, StringHash, stringComp, freeString);

  hashset answers;
  HashSetNew(&answers, sizeof(wordInfo),10007, wordHash,  wordCmp, freeWord);

  setbuf(stdout, NULL);
  curl_global_init(CURL_GLOBAL_DEFAULT);
  Welcome(kWelcomeTextFile);
  BuildIndices((argc == 1) ? kDefaultFeedsFile : argv[1], &set, &usedUrl, &answers);
  HashSetMap(&answers, SortMapFn, NULL);
  QueryIndices(&set, &answers);
  curl_global_cleanup();

  HashSetDispose(&set);
  HashSetDispose(&usedUrl);
  HashSetDispose(&answers);
  return 0;
}
...
static void BuildIndices(const char *feedsFileName, hashset *stopWords, hashset *usedUrl, hashset *answers)
{
  ...
  { // ignore everything up to the first selicolon of the line
    ...
    ProcessFeed(remoteFileName, stopWords, usedUrl, answers);
  }
...
static void ProcessFeedFromFile(char *fileName, hashset *stopWords, hashset *usedUrl, hashset *answers)
{
  FILE *infile;
  streamtokenizer st;
  ...
  ScanArticle(&st, (const char *)fileName, articleDescription,
              (const char *)fileName, stopWords, usedUrl, answers);
  STDispose(&st); // remember that STDispose doesn't close the file, since STNew
                  // doesn't open one..
  fclose(infile);
}
...
static void ProcessFeed(const char *remoteDocumentName, hashset *stopWords, hashset *usedUrl, hashset *answers)
{
  ...
  {
    ProcessFeedFromFile((char *)remoteDocumentName + strlen(kFilePrefix), stopWords, usedUrl, answers);
    return;
  }

  FILE *tmpFeed = FetchURL(remoteDocumentName, "tmp_feed");
  PullAllNewsItems(tmpFeed, stopWords, usedUrl, answers);
  fclose(tmpFeed);
}
...
static void PullAllNewsItems(FILE *dataStream, hashset* stopWords, hashset* usedUrl, hashset* answers)
{
  ...
  { // if true is returned, then assume that <item ...> has just been
    ...
    ProcessSingleNewsItem(&st, stopWords, usedUrl, answers);
  }
...
static void ProcessSingleNewsItem(streamtokenizer *st, hashset* stopWords, hashset* usedUrl, hashset* answers)
{
  ...
  ParseArticle(articleTitle, articleDescription, articleURL, stopWords, usedUrl, answers);
}
                         ...
                         const char *articleURL, hashset* stopWords, hashset* usedUrl, hashset* answers)
{
  ...
  ScanArticle(&st, articleTitle, articleDescription, articleURL, stopWords, usedUrl, answers);
  STDispose(&st);
  fclose(tmpDoc);
}

static bool visited(hashset* usedUrl, const char* url, const char* articleTitle){
  char* temp1 = strdup(url);
  char* temp2 = strdup(articleTitle);
  if(HashSetLookup(usedUrl, &temp1) || HashSetLookup(usedUrl, &temp2)){
    free(temp1);
    free(temp2);
    return true;
...
static int artCmpr(const void* elem1, const void* elem2){
  Article* art1 = (Article*)elem1;
  Article* art2 = (Article*)elem2;
  return strcmp(art1->url,art2->url);
}

static void freeArt(void* elem){
  Article* art = (Article*)elem;
  free(art->name);
  free(art->url);
}

static void add(char *word,  char *articleTitle, char *articleURL, hashset* answers){
  vector artVec;
  VectorNew(&artVec, sizeof(Article), freeArt, 10007);
  Article art;
  art.name = strdup(articleTitle);
  art.url= strdup(articleURL);
  art.words = 1;
  VectorAppend(&artVec, &art);

  wordInfo wrd;
  wrd.word = strdup(word);
  wrd.articles = artVec;
  HashSetEnter(answers, &wrd);
}

static void update(wordInfo* word, char* articleTitle, char* articleUrl){
  Article art;
  art.name = strdup(articleTitle);
  art.url = strdup(articleUrl);
  int ind = VectorSearch(&word->articles, &art, artCmpr, 0, false);

  if(ind == -1){
    art.words = 1;
    VectorAppend(&word->articles, &art);
    return;
  }

  Article* temp = (Article*)VectorNth(&word->articles, ind);
  temp->words++;
  free(art.name);
  free(art.url);
}
...
static void ScanArticle(streamtokenizer *st, const char *articleTitle,
                        const char *unused, const char *articleURL, hashset *stopWords, hashset *usedUrl, hashset *answers)
{
  char* lowerArt = strdup(articleTitle);
  int len = strlen(lowerArt);
  for(int i = 0; i < len; i++){
    lowerArt[i] = tolower(lowerArt[i]);
  }

  char* lowerUrl = strdup(articleURL);
  len = strlen(lowerUrl);
  for(int i = 0; i < len; i++){
    lowerUrl[i] = tolower(lowerUrl[i]);
  }

  if(visited(usedUrl, lowerArt, lowerUrl)) {
    free(lowerArt);
    free(lowerUrl);
    return;
  }

  HashSetEnter(usedUrl, &lowerUrl);
  HashSetEnter(usedUrl, &lowerArt);
  ...
  while (STNextToken(st, word, sizeof(word)))
  {
    ...
    else
    {
      ...
      if(HashSetLookup(stopWords,&curWord)){
        free(curWord);
        continue;
      } 
      ...
      {
        numWords++;
        wordInfo* temp = HashSetLookup(answers, &curWord);
        if(!temp){
         add(curWord, lowerArt, lowerUrl, answers);
        }else{
          update(temp, lowerArt, lowerUrl);
        }
...
static void QueryIndices(hashset* stopWords, hashset* answers)
{
  ...
  while (true)
  {
    ...
    if (strcasecmp(response, "") == 0)
      break;
    ProcessResponse(response, stopWords, answers);
  }
}
...
static void ProcessResponse(const char *word, hashset* stopWords, hashset* answers)
{
  if (WordIsWellFormed(word)) {
    ...
    wordInfo* curWord = HashSetLookup(answers, &word);
    if (curWord == NULL) {
       printf("None of today's news articles contain the word \"%s\".\n", word);
       return;
    }
    int numArticles = VectorLength(&curWord->articles);
    if(numArticles > 10) numArticles = 10;
    for (int i = 0; i < numArticles; i++) {
      Article* currArticle = VectorNth(&curWord->articles, i);
