...
def calculate_score(sentence):
    ...
    # pure_sentence = pure_sentence.lower()
    pure_sentence_parsed = ''.join(ch for ch in sentence if ch.isalpha() and (97 <= ord(ch) <= 122))
...
def find_sentence(input_hex):
    ...
    for try_key in range(256):
        ...
        dc = "".join(chr(b ^ try_key) for b in bytes_inp)
        sentence = bytes(xord).decode('utf-8', errors='ignore')
        score = calculate_score(dc)
        if score > cur_score:
            key = key
            cur_score = score
            cur_sentence = sentence

    return (cur_sentence, cur_score)
...
for n in range(int(inp)):
    ...
    (sent, score) = find_sentence(sentence)
    if score > cur_score:
        cur_sentence = sent
        cur_score = score
