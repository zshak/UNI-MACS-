...
func (kv *KVServer) ListenToApply() {
	for !kv.killed() {
		...
		if kv.maxraftstate != -1 && kv.persister.RaftStateSize() >= kv.maxraftstate {
			//log.Printf("%v -- %v --- %v", kv.persister.RaftStateSize(), kv.maxraftstate, kv.persister.RaftStateSize())
			kv.Snapshot(com.CommandIndex)
...
func (kv *KVServer) Snapshot(index int) {
	...
	err := encoder.Encode(Data{DB: kv.DB, LastReq: kv.LastReq})
	if err != nil {
		return
	}
...
func (kv *KVServer) InitializeFromSnapShot(snapshot []byte) {
	buffer := bytes.NewBuffer(snapshot)
	decoder := gob.NewDecoder(buffer)
	toDecodeTo := Data{}
	err := decoder.Decode(&toDecodeTo)
	if err != nil {
		return
	}
	kv.DB = toDecodeTo.DB
	kv.LastReq = toDecodeTo.LastReq
}
...
func StartKVServer(servers []*labrpc.ClientEnd, me int, persister *raft.Persister, maxraftstate int) *KVServer {
	...
	if maxraftstate != -1 && snapshot != nil && len(snapshot) > 0 {
		kv.InitializeFromSnapShot(snapshot)
	}
	return kv
}
