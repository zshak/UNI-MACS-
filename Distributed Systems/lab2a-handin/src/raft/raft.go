...
func (rf *Raft) RequestVote(args *RequestVoteArgs, reply *RequestVoteReply) {
	...
	if rf.votedFor < 0 || rf.votedFor == args.Candidate {
		...
		if args.LastLogTerm == lastLogTerm && len(rf.log) <= args.LastLogIndex {
			reply.VoteGranted = true
			rf.votedFor = args.Candidate
			rf.currentTerm = args.Term
			rf.state = FOLLOWER
...
func (rf *Raft) sendHeartBeat() {
	rf.mu.Lock()
	//log.Printf("sendHeartBeat: server %v aquired lock", rf.me)
	request := AppendEntriesArgs{Term: rf.currentTerm, LeaderId: rf.me}
	//log.Printf("sendHeartBeat: server %v unlocked lock", rf.me)
	rf.mu.Unlock()
